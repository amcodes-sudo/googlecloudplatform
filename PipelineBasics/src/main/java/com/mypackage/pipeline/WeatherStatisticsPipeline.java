...
public class WeatherStatisticsPipeline {
    ...
    public static class PrintStringToConsole extends DoFn<String, Void> {
        @ProcessElement
    ...
    public static class ConvertCsvToWeatherRecord extends DoFn<String, WeatherRecord> {
        ...
        public void processElement(@Element String line, OutputReceiver<WeatherRecord> o) throws Exception {
            ...
            String dummy = rowValues[7];
            WeatherRecord record = new WeatherRecord(locId, lat, lng, date, lowTemp, highTemp, precip, dummy);
            o.output(record);
        }
    }
    ...
    public static class ComputeStatistics extends PTransform<PCollection<WeatherRecord>, PCollection<String>>{
        ...
        public PCollection<String> expand(PCollection<WeatherRecord> pcoll){
            ...
            final SerializableFunction<WeatherRecord,Boolean> filterCondition = weatherRecord -> {
                    if(weatherRecord!=null && weatherRecord.highTemp.doubleValue()>110)
                        return true;
                    return false;
            };

            //PCollection<WeatherRecord> pcollDistinct = pcoll.apply("Distinct",Group.<WeatherRecord>byFieldNames(""));

            PCollection<String> weatherStats =
                    pcoll.apply("Filter",Filter.by(filterCondition))
                            .apply("Aggregations", Group.<WeatherRecord>byFieldNames("locId")
    ...
    public static void main(String[] args) {
        ...
        options.setInputPath("src/main/resources/test.csv");
    ...
    public static PipelineResult run(Options options) {
        ...
        pipeline
                ...
                .apply("Write to Console",ParDo.of(new PrintStringToConsole()));
